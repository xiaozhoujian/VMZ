...
def train_main(args):
    ...
    # automatically detect whether is using distribution mode
    ...
    # set a tensorboard in output_dir
    ...
    if not args.eval_only:
        ...
        transform_train = torchvision.transforms.Compose(
            [
                # convert tensor data type from uint 8 to float, divide value by 255.0
                # and permutep dimensions of clip tensor
                # from clip(torch.tensor, dtype=torch.uint8): Size is (Time, Height, Width, Channel) to (C, T, H, w)
                T.ToTensorVideo(),
                # resize the clip to (scale_h, scale_w), remains the ration of height/width to the original one
                T.Resize((args.scale_h, args.scale_w)),
                # 50% to do a random horizontal flip to these clips
...
if __name__ == "__main__":
    ...
    import torchvision
    # There are two backend, pyav and video_reader. And video_reader is faster than pyav
    torchvision.set_video_backend("video_reader")
    args = parse_args()
    train_main(args)
    exit()
